...
- `experiments/texture_baker/textured_mesh_preview.py` renders private quick previews by combining the observed atlas, Pixel3DMM `flame_uv_coords.npy`, and the frozen PLY meshes. Current correct orientation is `--uv-mode flip_y --depth-mode max`. It also supports review-only FLAME-mask material fallback for black sampled texels.
...
- Current recommended private review sheet: `output/_comparison/face_texture_model_comparison_8view_material_fallback.png` at 4496x3296, plus JSON manifest beside it. It contains 6 rows: Juseop 3 candidates and Eunchae 3 candidates, each at yaw `0,45,90,135,180,225,270,315`, with black holes filled by simple scalp/neck/ear/lip/eyeball fallback colors.
