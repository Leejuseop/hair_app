...
1. Mask-aware FaceBuilder texture correction:
   - keep FaceBuilder geometry/camera alignment;
   - extract camera/mesh/UV data from the saved `.blend`;
   - verify mesh reprojection over the input crops;
   - compute UV visibility/source-count maps;
   - build parser/object masks for `usable_skin`;
   - use these maps later to project only clean skin pixels back into UV.

2. Parser and occlusion ablation:
   - `v0_farl_only`: existing Pixel3DMM/FaRL parser baseline;
   - `v1_facexformer_only`: FaceXFormer parser baseline from Colab;
   - `v2_farl_grounded_sam`: FaRL plus object/occlusion masks from Grounded SAM2;
   - `v3_facexformer_grounded_sam`: FaceXFormer plus Grounded SAM2.

3. Semantic post-bake processing:
...
- `experiments/facebuilder_mask_aware_correction/README.md` for the active
  mask-aware correction experiment.

### 21.7 FaceBuilder mask-aware correction status
...
- `v2_farl_grounded_sam` waits for Grounded SAM2 object masks.
- `v3_facexformer_grounded_sam` waits for both external outputs.

The Colab cells for external masks live in:
...
- Current working mask candidate:
...
- Grounded SAM is used conservatively. Broad face/head/hair detections and
  oversized masks are rejected before entering `usable_skin`.

Step 4 clean-pixel UV projection has also been implemented. This stage uses
FaceBuilder's solved mesh, camera, and UV data to project only Step 3
`usable_skin` pixels into the texture atlas. It writes clean projected textures,
coverage maps, confidence maps, source-count maps, per-camera contribution
maps, atlas review sheets, and yaw render review sheets.
...
Observed Step 4 coverage:

- Juseop texture-camera-only: about 19.9% clean atlas coverage.
- Juseop with alignment/scan cameras: about 23.2% clean atlas coverage.
- Eunchae: about 15.5% clean atlas coverage in both runs.

Important interpretation:

- Step 4 proves clean-pixel reprojection is feasible.
- Step 4 is not a final texture. It intentionally leaves untrusted/unknown
  regions sparse.
- Step 5 now arbitrates raw FaceBuilder texture versus Step 4 projected raw
  texture, then marks unresolved regions for completion.
...
- COMPLETION_NEEDED pixels are black in actual output textures.
- Decision-map colors:
  - red = CLEAN_ONLY
  - blue = RAW_ONLY
  - green = BOTH_OK
...
Observed Step 5 category ratios:
...
Important Step 6 v04b result:

- v04b keeps the v04 forehead-region idea but fixes the obvious Juseop eyebrow
  failure seen in review.
- It strengthens the eye/eyebrow guard with component-scored source selection
  instead of area-based left/right selection. If one side is good and the other
  side is bad or missing, the bad/missing side is discarded and the good
  component is mirrored as a fixed black symmetric mask. This stage does not
  attempt final eyebrow texture or color transfer.
- It also runs a second-pass hairline correction: the front hairline curvature
  is reduced so it is less circular, and if reliable forehead-skin pixels exist
  above the first smooth hairline, the front segment is broadly lifted
  symmetrically so valid forehead skin is not cut off on only one side.
- It defines the editable forehead as below the final hairline and above the
  eyebrow baseline, except tight eye/eyebrow guards. This replaces the
  central-component-only behavior that left temple-side forehead remnants
  unedited.
- Juseop scan hairline frames remain boundary-only. Scan pixels are still
  never used as texture/color/bake input.
- v04b metrics:
  - Juseop: 20,252 forehead-region texels, 6,826 hair/black fill texels,
    1,708 symmetric eyebrow texels, max hairline lift 48.63 px. The selected
    brow source is the left `61x16` component with score `4.85`.
  - Eunchae: 22,053 forehead-region texels, 10,223 hair/black fill texels,
    1,838 symmetric eyebrow texels, max hairline lift 62.17 px. The selected
    brow source is the right `63x16` component with score `5.00`.
- Visual review says v04b prevents Juseop's missing eyebrow side from being
  swallowed by forehead fill without copying the larger bad blob as the source,
  and places the front hairline as a flatter symmetric broad lift instead of
  isolated one-sided bumps. The eyebrow-baseline definition reduces upper
  hairline/temple black remnants, but side-face/ear artifacts remain separate
...
- After user review, v05 is retained as a logged experiment but is not the
  current visual baseline. The side/neck result looked too odd, so v06 restarts
  from v04b and applies a simpler bald-head rule.

Important Step 6 v06 result:

- v06 starts from v04b, not from v05.
...
4. Simple bald-head temporary skin fill
   - v06 is the current postprocess baseline.
   - It blacks out everything above the refined hairline because that region is
     future hair/scalp territory for the hair app.
   - It fills bad below-hairline neck/face/side pixels with simple temporary
     skin while preserving usable pixels and tight feature guards.
   - It is not final skin quality and should not be judged as the final neck,
     mouth, eye, or eyebrow material.
